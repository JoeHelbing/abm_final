...
for i, p in enumerate(params):
    ...
    if not os.path.exists(f"{path}/model/seed_{list(batch_step_model_raw.keys())[0][0]}"):
        os.makedirs(f"{path}/model/seed_{list(batch_step_model_raw.keys())[0][0]}")
    for key, df in batch_step_model_raw.items():
        df.to_csv(f"{path}/model/seed_{key[0]}/model_seed_{key[0]}_pp_{key[1]}_sd{key[2]}_ep_{key[3]}.csv")

    if not os.path.exists(f"{path}/agent/seed_{list(batch_step_agent_raw.keys())[0][0]}"):
        os.makedirs(f"{path}/agent/seed_{list(batch_step_agent_raw.keys())[0][0]}")
    for key, df in batch_step_agent_raw.items():
        df.to_csv(f"{path}/agent/seed_{key[0]}/agent_seed_{key[0]}_pp_{key[1]}_sd{key[2]}_ep_{key[3]}.csv")
